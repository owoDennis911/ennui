...
public class BlockUtil implements Globals
{
    ...
    /**
     * Generates a list of block positions in a spherical shape around a center position.
     * This method does not check for block validity; it simply returns positions within the sphere.
     *
     * @param centerPos The center position of the sphere.
     * @param range The horizontal radius of the sphere.
     * @param yRange The vertical radius of the sphere.
     * @param hollow If true, only blocks on the outer edge of the sphere are included.
     * @return A sorted list of block positions in a sphere around the center.
     */
    public static List<BlockPos> getSpherePositions(BlockPos centerPos, float range, float yRange, boolean hollow) {
        List<BlockPos> positions = new ArrayList<>();
        int radiusX = (int) Math.ceil(range);
        int radiusY = (int) Math.ceil(yRange);
        ...
        for (int x = -radiusX; x <= radiusX; x++) {
            for (int y = -radiusY; y <= radiusY; y++) {
                for (int z = -radiusX; z <= radiusX; z++) {
                    ...
                    double distanceSq = centerPos.distanceSq(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5);
                    double radiusSq = range * range;

                    if ((hollow && distanceSq >= (radiusSq - range) && distanceSq < radiusSq) ||
                            (!hollow && distanceSq < radiusSq)) {
                        positions.add(pos);
                    }
                }
            }
        ...
        positions.sort(Comparator.comparingDouble(pos ->
                centerPos.distanceSq(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5)
        ));
