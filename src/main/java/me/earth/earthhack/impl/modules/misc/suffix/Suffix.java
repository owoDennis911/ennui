...
public class Suffix extends Module
{
    ...
    protected final StringSetting custom =
            register(new StringSetting("Custom", "ennui"));
    protected final StringSetting separator =
            register(new StringSetting("Separator", " | "));
    /*
    ...
    public Suffix()
    {
        ...
        this.listeners.add(new LambdaListener<>(PacketEvent.Send.class, e -> {
            if (e.getPacket() instanceof CPacketChatMessage) {
                ...
                if (!message.startsWith("/") && !message.startsWith(getPrefix()) && !message.endsWith(ENNUI) &&!message.endsWith(EARTH) && !message.endsWith(CUTEEARTH) && !message.endsWith(PHOBOS) && !message.endsWith(RUSHER) && !message.endsWith(custom.getValue()) && !message.endsWith(FUTURE) && !message.endsWith(KONAS) && !message.endsWith(GAMESENSE) && !message.endsWith(KAMIBLUE) && !message.endsWith(LEMON)) {
                    e.setCancelled(true);
                    mc.player.connection.sendPacket(new CPacketChatMessage(getSuffix(message)));
                } else if (msgs.getValue() && !message.startsWith(getPrefix()) &&!message.startsWith(getPrefix()) && !message.endsWith(ENNUI) && !message.endsWith(EARTH) && !message.endsWith(CUTEEARTH) && !message.endsWith(PHOBOS) && !message.endsWith(RUSHER) && !message.endsWith(custom.getValue()) && !message.endsWith(FUTURE) && !message.endsWith(KONAS) && !message.endsWith(GAMESENSE) && !message.endsWith(KAMIBLUE) && !message.endsWith(LEMON)) {
    ...
    protected static final String KONAS = "\uff2b\uff4f\uff4e\uff41\uff53";
    protected static final String GAMESENSE = "\u0262\u1D00\u1D0D\u1D07\uA731\u1D07\u0274\uA731\u1D07";
    protected static final String KAMIBLUE = "\u1D0B\u1D00\u1D0D\u026A \u0299\u029F\u1D1C\u1D07";
    protected static final String ENNUI = "\uff45\uff4e\uff4e\uff55\uff49";
    protected static final String LEMON = "ℓємℴภ";
    ...
    public String getSuffix(String message)
    {
        ...
        if (mode.getValue() == Mode.Earth) {
            ...
            suffix = KAMIBLUE;
        } else if (mode.getValue() == Mode.ennui) {
            suffix = ENNUI;
        } else if (mode.getValue() == Mode.Lemon) {
            suffix = LEMON;
        }  else if (mode.getValue() == Mode.Custom) {
            suffix = custom.getValue();
        }
        return message + " " + separator.getValue() + " " + suffix;
    }
}